...
SECTION_TABLE_ENTRY_CODE:
  ...
  dd SECTION_TABLE.IMAGE_SCN_CNT_CODE \
   ...
   + SECTION_TABLE.IMAGE_SCN_MEM_READ ; u32 Characteristics
...
SECTION_TABLE_ENTRY_IDATA:
  ...
  dw 0                                              ; u16  NumberOfRelocations
  dw 0                                              ; u16  NumberOfLineNumbers
  dd SECTION_TABLE.IMAGE_SCN_CNT_INITIALIZED_DATA \
...
start.RVA = (start-CODE)+CODE.RVA
  ...
  ; Store my_value[i] (i << 1)
  sub r12d, r12d
  lea r13, [ rip + (my_value-((@f-CODE)+CODE.RVA)) ]
  @@:
  store_next:
  ...
  ; Print values stored in my_value (really only printing bottom 32bits of each qword)
  sub r12d, r12d
  lea r13, [ rip + (my_value-((@f-CODE)+CODE.RVA)) ]
  @@:
  print_next:
  ...
  print_1    db '%p: my_value[%d] = %d',0x0a,0
...
BSS:
BSS.RVA = IDATA.RVA + IDATA_IMAGE_SIZE
print_value_x32 "BSS.RVA        = ", BSS.RVA

  my_value = ($-BSS)+BSS.RVA ; RVA
  rq 1024
